...
- The current PhysioNet and BNCI2014-001 fold-level CSV files were regenerated with protocol version 0.3.2. They record `protocol_version`, `mask_seed_scope`, and one `run_signature` per dataset–pipeline run. Random-dropout rows use participant-specific masks; non-random stressors are marked `not_applicable`.
