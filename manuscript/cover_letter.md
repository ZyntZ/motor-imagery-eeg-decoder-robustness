...
The repository contains the analysis code, participant-level tables and commands used to reproduce the reported comparisons. The PhysioNet dropout estimates are conditional on one cross-validation split and deterministic participant-specific mask schedules, matched between decoder families within participant. The manuscript does not claim stability across alternative splits or schedules. The study is a secondary analysis of public, de-identified datasets and received no external funding.
